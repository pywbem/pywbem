...
(max_ind_queue_size) that causes generation of an exception (ListenerQueueFull)
and stopping of sending indication reception. Since this effectively stops the
listener, it should only be used if the number of indications in the queue is a
threat to the memory in the client, not for temporary slowdown of the flow of
indications from the indication sender.
...
class WBEMListener:
    ...
    *New in pywbem 0.9 as experimental and finalized in 0.10.*
    ...
    def __init__(self, host, http_port=None, https_port=None,
        ...
        Parameters:
          ...
          max_ind_queue_size (:term:`integer`):
            ...
            blocked and the QueueFull exception is executed. No more
            indications will be accepted from the indication sender.
    ...
    def max_ind_queue_size(self):
        """
        Return the maximum queue size. If this value is not zero, an
        exception will be generated when this queue size is reached.
        """
    ...
    def ind_delivery_queue_empty(self):
        ...
        False. This is available becasue the qsize attribute only returns an
        approximation.
        """
    ...
    def qsize(self):
        """
        Return the approximate count of the number of indications currently in
        the received indication queue for this listener.
        """
    ...
    def stop_indication_delivery(self, force=False):
        """
        Stop the indication delivery thread and the queue and remove them. This
        allows the delivery of indications already in the queue. and then
        closes the callback_thread and indication delivery queue.

        Force causes the queue to be removed without calling callback for
        indications
        """
        ...
        force = True  # Future: Allow override of force to fast clear queue
        if not force:
    ...
    def deliver_indications_forever(self, ind_queue):
        ...
        while True:
            try:
                self.logger.debug(
                    "Get from queue. qsize=%d", ind_queue.qsize())
                ...
                # this simple case of a single worker.
                ind_queue.task_done()
    ...
    def handle_indication(self, indication, host):
        ...
        try:
            ...
            self.logger.debug(
                "Rcvd indication queue put done. qsize #%s",
                self.ind_delivery_queue.qsize())

        except queue.Full:
            qsize = self.ind_delivery_queue.qsize()
            self.logger.debug(
                "Rcvd indication queue full. ListenerQueueFull Exception, "
                "qsize = #%s", qsize)
            new_exc = ListenerQueueFullError(
                f"Listener ind delivery queue full, qsize = {qsize}. "
                f"Closing listener.")
            new_exc.__cause__ = None
