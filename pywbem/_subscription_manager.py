...
"""
The :class:`~pywbem.WBEMSubscriptionManager` class provides for creating and
removing indication subscriptions (including filters and listener destinations)
for multiple WBEM servers and multiple WBEM listeners and for getting
information about existing indication subscriptions.

This class supports several types of indication subscriptions:
...
* **Not-owned dynamic filters, destinations, and subscriptions** -
  ...
  removed.  Dynamic filters are those that can be created by clients.
  It is the user responsibility to eventually remove these
...
* **Static filters** - These are filters that are statically
  defined in the server. They can be inspected through
...
class WBEMSubscriptionManager(object):
    ...
    def add_listener_destinations(self, server_id, listener_urls, owned=True):
        ...
        This function creates a destination instance for each specified
        listener in `listener_urls` and registers that instance with the
        ...
        Parameters:
          ...
          listener_urls (:term:`string` or list of :term:`string`):
            Either a single listener URL or list of listener URLs. This
            represents the listeners for which indications will be routed and
            for which destinations instances are registered with the WBEM
            server.
          ...
          owned (:class:`py:bool`)
            ...
            If `True`, these listener destination instances will have a life
            ...
            If `False`, these listener destination instances will have a life
            cycle that is independent of the registration of the WBEM server
            within this subscription manager. The user is responsible for
            deleting these listener destination instances explicitly (for
            example by using
        ...
        *static indication filters* and cannot be removed by clients.
        ...
        Parameters:
          ...
          owned (:class:`py:bool`)
            ...
            If `True`, these filter instances will have a life cycle that is
            bound to the registration of the WBEM server within this
            subscription manager. The user does not need to take care of
            deleting these instances in the WBEM server. Instead, if the WBEM
            server is deregistered from this subscription manager
            (see :meth:`~pywbem.WBEMSubscriptionManager.remove_server` and
            :meth:`~pywbem.WBEMSubscriptionManager.remove_all_servers`), these
            ...
            If `False`, these filter instances will have a life cycle that is
            independent of the registration of the WBEM server within this
            subscription manager. The user is responsible for deleting these
            filter instances explicitly (for example by using
    ...
    def add_subscriptions(self, server_id, filter_path, destination_paths=None,
        ...
        Parameters:
          ...
          owned (:class:`py:bool`)
            ...
            If `True`, these subscription instances will have a life cycle that
            is bound to the registration of the WBEM server within this
            subscription manager. The user does not need to take care of
            deleting these subscription instances in the WBEM server. Instead,
            if the WBEM server is deregistered from this subscription manager
            (see :meth:`~pywbem.WBEMSubscriptionManager.remove_server` and
            :meth:`~pywbem.WBEMSubscriptionManager.remove_all_servers`), these
            subscription instances will be automatically deleted in the WBEM
            server.

            If `False`, these subscription instances will have a life cycle
            that is independent of the registration of the WBEM server within
            this subscription manager. The user is responsible for deleting
            these subscription instances explicitly (for example by using
            :meth:`~pywbem.WBEMSubscriptionManager.remove_subscriptions`).
