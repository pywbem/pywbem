...
# In addition to the files specified here, the vendorized packages are added
# when the MANIFEST.in file is created.
...
help:
	...
	@echo "  vendorize  - Create the vendorizd packages in: $(vendor_dir)"
...
install_pywbem_$(pymn).done: Makefile pip_upgrade_$(pymn).done vendorize_$(pymn).done requirements.txt setup.py
	-$(call RM_FUNC,$@)
...
installc_pywbem_$(pymn).done: Makefile pip_upgrade_$(pymn).done vendorize_$(pymn).done $(bdistc_file)
	-$(call RM_FUNC,$@)
ifdef TEST_INSTALLED
	@echo "Makefile: Skipping installation of pywbem and its Python runtime prerequisites because TEST_INSTALLED is set"
	@echo "Makefile: Checking whether pywbem is actually installed:"
...
develop_$(pymn).done: pip_upgrade_$(pymn).done install_$(pymn).done develop_os_$(pymn).done install_basic_$(pymn).done dev-requirements.txt vendorize-requirements.txt test-requirements.txt
...
clobber: clean
	...
	-$(call RMDIR_FUNC,$(doc_build_dir) .tox $(coverage_html_dir) build_build_ext build_cythonize $(vendor_dir))
...
.PHONY: vendorize
vendorize: vendorize_$(pymn).done
	@echo "Makefile: Target $@ done."
...
MANIFEST.in: Makefile vendorize_$(pymn).done $(dist_manifest_in_files)
...
ifeq ($(PLATFORM),Windows_native)
	...
	echo "recursive-include $(vendor_dir) *" >>$@
	echo "recursive-exclude $(vendor_dir) *.pyc" >>$@
	echo "recursive-include $(test_dir) *" >>$@
	echo "recursive-exclude $(test_dir) *.pyc" >>$@
	@echo "Makefile: Done creating the manifest input file: $@"

# Vendoring packages
vendorize_$(pymn).done: install_basic_$(pymn).done vendorize.toml vendorize-requirements.txt
	@echo "Makefile: Creating vendorized packages (with PACKAGE_LEVEL=$(PACKAGE_LEVEL))"
	-$(call RM_FUNC,$@)
	-$(call RMDIR_FUNC,$(vendor_dir))
	$(PIP_INSTALL_CMD) $(pip_level_opts) -r vendorize-requirements.txt
	python-vendorize vendorize.toml
	echo "done" >$@
	@echo "Makefile: Done creating vendorized packages"
...
$(sdist_file): setup.py MANIFEST.in vendorize_$(pymn).done $(dist_dependent_files) $(moftab_files)
...
$(bdist_file): setup.py MANIFEST.in vendorize_$(pymn).done $(dist_dependent_files) $(moftab_files)
...
$(bdistc_file): setup.py MANIFEST.in vendorize_$(pymn).done $(dist_dependent_files) $(moftab_files)
...
$(moftab_files): install_$(pymn).done vendorize_$(pymn).done $(moftab_dependent_files) build_moftab.py
...
pylint_$(pymn).done: develop_$(pymn).done Makefile $(pylint_rc_file) vendorize_$(pymn).done $(py_src_files) $(py_test_files)
...
flake8_$(pymn).done: develop_$(pymn).done Makefile $(flake8_rc_file) vendorize_$(pymn).done $(py_src_files) $(py_test_files)
ifeq ($(python_mn_version),2.6)
...
todo_$(pymn).done: develop_$(pymn).done Makefile $(pylint_rc_file) vendorize_$(pymn).done $(py_src_files) $(py_test_files)
ifeq ($(python_m_version),2)
