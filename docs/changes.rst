...
* Update to support python urllib3 package version 2.0+ (for python versions 3.7+)
  opens the SSL library implementations (ex. OpenSSL, libreSSL)
  that requuire TLS >=1.2 (OpenSSL versions >= 1.1.1). If an older
  OpenSSL or incompatible SSL library is used, the import of urllib3 will
  cause an import error. The actual OpenSSL version is determined by the SSL library
  installed (i.e OpenSSL version 1.1.1) , not   by the urllib3 version. When the 
  server TLS versions allowed by the server are   not compatible with the pywbem 
  client an exception like the following may occur:
  ...
  Also, the default set of cyphers available in OpenSSL 1.1.1 has changed to use the 
  list of ciphers configured by the system rather than ciphers defined by urllib3.
