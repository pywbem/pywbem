...
def parse_keyvalue(tt):
    ...
    check_node(tt, 'KEYVALUE', [], ['VALUETYPE'], [], True)

    vt = attrs(tt).get('VALUETYPE', 'string')
    p = pcdata(tt)
    if vt == 'string':
        return p
    elif vt == 'boolean':
...
def parse_qualifier(tt):
    ...
    q = cim_obj.CIMQualifier(a['NAME'], unpack_value(tt))
...
def parse_paramvalue(tt):
    ...
    check_node(tt, 'PARAMVALUE', ['NAME'], ['PARAMTYPE'])
